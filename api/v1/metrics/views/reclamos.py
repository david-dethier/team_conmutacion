...
from api.v1.metrics.serializers.reclamos import (
    CompletedWorkClaimsSerializer,
)

URL_COMPLETEDWORKS = "http://192.168.36.36/Recladisticas/estadisRec.php"


class CompletedWorkClaimViewSet(viewsets.ReadOnlyModelViewSet):

    serializer_class = CompletedWorkClaimsSerializer
    ...
    def get_queryset(self):
        ...
        return ReclamosCompletedWorkModel.objects.all()
    ...
    def sincronize(self):
        ...
        for teamworks_data in json_request_data:
            ...
            # technicalteam_name, name_is_valid = TechnicalTeamMapper().key_to_namedb(
            #     key=teamworks_data["cuadrilla"]
            # )
            technicalteam_name = teamworks_data["cuadrilla"]
            # TECHNICALTEAMS[teamworks_data["cuadrilla"]]

            # if not name_is_valid:
            if not technicalteam_name:
                raise serializers.ValidationError(
                    technicalteam_name, status.HTTP_400_BAD_REQUEST
                )
            ...
            model, created = ReclamosCompletedWorkModel.objects.update_or_create(
